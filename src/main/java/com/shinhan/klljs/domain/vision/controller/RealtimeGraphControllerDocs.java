...
public interface RealtimeGraphControllerDocs {
    ...
    @Operation(
            ...
            description = """
                    ...
                    ### `points` 배열
                    - **여러 날짜를 조회하면 날짜별로 나뉘지 않고 시각(0~23시)별로 합산된다.** 예를 들어
                      07-01~07-05를 조회하면 5일치 06시 데이터를 전부 더한 "06시" 포인트 하나가 나온다 -
                      배열 길이는 조회 기간의 날짜 수와 무관하게 항상 최대 24개(0~23시)다.
                    - `points[].eventTime`: 날짜 부분은 `effectivePeriod.startDate`로 고정된 값이고, 실제 값은
                      그 시각에 해당하는 조회 기간 내 모든 날짜를 합산한 것이다 - 특정 하루만의 데이터가 아니다.
                    - 데이터가 없는 시각은 배열에서 통째로 빠진다 (0으로 채워서 내려주지 않음 - 5-1과 동일한 방식).
                    - `effectivePeriod`에 해당하는 시각만 포함한다. `BEFORE_EXECUTION`이면 `points: []`.
                    """
    )
    @ApiResponses({
            @ApiResponse(
