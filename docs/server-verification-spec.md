...
> ⚠️ **이 문서의 기능은 MVP 범위에서 제외되어 보류 중이다 (2026-07 팀 결정).** 국세청
> 진위확인·광고업 분류·자동 판정은 구현 PR과 함께 보류되었고, MVP는 OCR 결과를 사용자가
> 확인하면 그대로 팀을 만든다 (`team-creation-api-spec.md` 참고). 이 문서는 기능을 다시 살릴
> 때를 위한 설계 기록으로 남긴다.
...
> **⚠️ 9번과 `NOT_CHECKED`는 현재 운영 경로에서 도달하지 않는다. 그래도 지우지 말 것.**
> 서버는 항상 `/validate`를 호출하므로 진위 결과는 `VALID`/`INVALID` 둘뿐이고, 9번은 죽은 코드처럼 보인다.
> 이 둘은 §5.1의 **폐업 재검증 배치**를 위해 남겨둔 것이다 — 그 배치는 승인된 사업자의 폐업 여부만
> 다시 확인하면 되고, 대표자명·개업일을 사용자에게 다시 물을 수 없으니 `/status`만 호출한다.
> 그때 진위는 확인되지 않은 상태로 판정에 들어온다.
> 지우고 `VALID`로 뭉뚱그리면 **진위확인 없이 승인이 유지되는 경로**가 열리고, `INVALID`로 뭉뚱그리면
> 멀쩡한 사업자에게 "진위확인 실패"라는 잘못된 사유가 나간다.
> `VerificationDecisionEvaluatorTest.evaluate_certificateNotChecked_isReviewRequiredNotAccepted()`가
> 이 경로를 고정하고 있다.
...
`team_id`가 UNIQUE라 UPSERT가 UPDATE로 동작한다. 그대로 두면 **이미 `APPROVED`인 팀이 잘못된 문서로 재제출했을 때 승인 상태가 `REJECTED`로 덮인다.** ERD가 이력 미보관을 의도했더라도, 현재 상태가 퇴행하는 것은 별개의 문제다.

**해결됨 — 이미 `APPROVED`인 팀의 재제출은 `409`(`BUSINESS_409_001`)로 막는다.** 승인된 사업자 정보의 변경은 운영자를 거치도록 한다. 사전 검사와 쓰기 트랜잭션 안 재확인 두 곳에서 확인한다(동시 요청 대비).
...
사용자가 OCR만 하고 이탈했다 돌아오면 **파일을 다시 업로드해야 한다.** 기존 S3 키로 OCR만 다시 돌리는 엔드포인트는 없다 — 업로드 API가 곧 OCR API이기 때문이다. 재업로드는 업로드 횟수 제한(24시간 10회)을 소모한다.

참조되지 않은 채 남는 파일은 이 설계 당시에는 정리 배치가 지울 예정이었으나, **배치가 MVP에서
제외되어 지금은 운영자가 수동으로 정리한다** (team-creation-api-spec.md 10절 "이번 범위에서 제외").

---

## 7. 검증하지 않는 값

`companyName`, `businessType`, `businessItem`은 data.go.kr가 확인해주지 않는다.

**업태(`businessType`)·종목(`businessItem`)은 폼으로 받지 않는다.** 이 둘은 광고업 분류의 유일한 근거인데, 사용자가 고칠 수 있으면 실제로는 음식점업인 사람이 "광고대행"으로 바꿔 스스로를 승인시킬 수 있다. 그래서 요청 DTO에 **필드 자체를 두지 않고**, 업로드 API가 OCR로 읽은 원본을 `documentStorageKey` 토큰 payload에 **HMAC 서명해서** 넣어 나른다. 값을 바꾸면 서명 검증이 깨진다.

대가로 **OCR이 업태·종목을 못 읽으면 사용자가 손으로 보완할 방법이 없다** — 화면에 입력란이 없기 때문이다. 이 경우 `review_required`(`ADVERTISING_CLASSIFICATION_INPUT_INCOMPLETE`)가 되고, 사용자는 문서를 다시 업로드하는 수밖에 없다. (이 막다른 길은 검증 기능과 함께 보류됐다 — 현행 MVP에서는 업태·종목을 못 읽어도 null로 저장될 뿐 팀 생성이 막히지 않는다. team-creation-api-spec.md 4절 참고.)

남는 것은 `companyName`(사업자명)뿐이다. 이 값은 사용자가 자유롭게 고칠 수 있지만 **판정에 전혀 쓰이지 않는다** — 표시·식별용으로만 저장된다. 서버는 `document_storage_key`로 원본 문서를 보관하므로 사후 감사도 가능하다.
